...
# Best-effort height guess, not visually confirmed — this StaticBody3D's
# own CollisionShape3D (in this node's local space) tops out around local
# y ≈ 1.40, so this sits a bit above that. Nudge this if it ends up
# buried in/clipping the desk mesh in-game.
const ALERT_MARKER_Y: float = 1.9
...
# in-popup Help-button pulse (see IntelConsolePopup.attention_changed):
# that one only nudges a button nobody sees until they've already opened
# the console. This is visible from across the deck instead.
# -------------------------------------------------------
